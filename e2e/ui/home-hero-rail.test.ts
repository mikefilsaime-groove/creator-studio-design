  ...
  },
  {
    ...
    manifest: {
      ...
      od: {
        ...
        useCase: {
          query:
            'Create refreshable, auditable Creator Studio Design artifacts backed by connector or local data.',
        },
      },
    },
  },
...
async function waitForLoadingToClear(page: Page) {
  await page.getByText('Loading Creator Studio Design…').waitFor({ state: 'hidden', timeout: 15_000 });
}
...
test('[P1] selecting another example updates the composer input', async ({ page }) => {
  ...
  await expect(input).toHaveText('Create refreshable, auditable Creator Studio Design artifacts.');
});
