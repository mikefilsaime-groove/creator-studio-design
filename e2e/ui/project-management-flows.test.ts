...
test('[P0] UI-created Personal project recovers preview and write authority after reload', async ({ page }) => {
  ...
  try {
    ...
    // (`Loading Creator Studio Design…`) and the project-route workspace-context gate
    ...
    await page
      .getByText('Loading Creator Studio Design…')
...
async function wireTeamRunBalanceFixtures(
  ...
  await routeAgents(page, [
    ...
    {
      id: 'amr',
      name: 'Creator Studio Design Cloud',
      bin: 'amr',
      available: true,
      version: 'cloud',
...
async function dismissPrivacyDialog(page: Page) {
  const privacyRegion = page.getByRole('region', { name: /Help us improve Creator Studio Design/i });
