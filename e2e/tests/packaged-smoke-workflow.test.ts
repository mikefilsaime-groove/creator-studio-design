...
`,
  ...
  it("[P1] smokes the published prerelease artifact, not the build directory", async () => {
    ...
    expect(stage).toContain('join(toolsPackDir, "out", "mac", "namespaces", namespace, "dmg", `Creator Studio Design-${token}.dmg`)');
    expect(stage).toContain('join(toolsPackDir, "out", "win", "namespaces", namespace, "builder", `Creator Studio Design-${token}-setup.exe`)');
  ...
  it("rejects stale latest platform manifests from a previous beta version", async () => {
    ...
    try {
      ...
      await writeFile(
        ...
        artifacts: {
          dmg: {
            url: "https://releases.open-design.ai/betas/versions/1.2.3-beta.3.unsigned/Creator Studio Design Beta.dmg",
          },
        },
        channel: "beta",
        github: {
  ...
  it("rejects stale latest platform manifests from a previous same-version beta workflow run", async () => {
    ...
    try {
      ...
      await writeFile(
        ...
        artifacts: {
          dmg: {
            url: "https://releases.open-design.ai/betas/versions/1.2.3-beta.4.unsigned/Creator Studio Design Beta.dmg",
          },
        },
        channel: "beta",
        github: {
  ...
  return {
    ...
    platforms: {
      mac: {
        arch: "arm64",
        artifacts: {
          dmg: artifact("Creator Studio Design.dmg"),
          zip: artifact("Creator Studio Design-mac-arm64.zip"),
        },
        enabled: true,
        signed: true,
      },
      macIntel: {
        arch: "x64",
        artifacts: {
          dmg: artifact("Creator Studio Design Intel.dmg"),
          zip: artifact("Creator Studio Design-mac-x64.zip"),
        },
        enabled: true,
        signed: true,
      },
      win: {
        arch: "x64",
        artifacts: {
          installer: artifact("Creator Studio Design Setup.exe"),
        },
        enabled: true,
      },
    },
