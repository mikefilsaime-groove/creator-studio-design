...
export function buildVersionDiagnostic(
  ...
  if (!version) {
    return {
      ...
      severity: 'error',
      message: `${def.name} was found, but Creator Studio Design could not verify its version.`,
  ...
  return {
    ...
    severity: 'warning',
    message: `${def.name} ${version} has not been tested with this Creator Studio Design build.`,
...
export function buildCompatibilityDiagnostic(
  ...
  return {
    ...
    severity: 'error',
    message: `${def.name} is installed, but its Creator Studio Design profile is missing or incompatible.`,
    detail: 'Install the pinned Creator Studio Design profile bundle in Harness profile `open-design`, then rescan.',
    fixActions: [
      { kind: 'openDocs' },
      { kind: 'rescan' },
    ],
