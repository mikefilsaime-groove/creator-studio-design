...
function fallbackComposioDescription(name: string, category: string | undefined): string {
  ...
  if (normalizedCategory?.includes('project')) return `Coordinate ${name} projects, tasks, and workflow data inside Creator Studio Design.`;
  if (normalizedCategory?.includes('communication')) return `Bring ${name} conversations, channels, and collaboration context into Creator Studio Design.`;
  if (normalizedCategory?.includes('documentation')) return `Search and reuse ${name} knowledge, pages, and documentation in Creator Studio Design.`;
  if (normalizedCategory?.includes('storage')) return `Find and reference ${name} files, folders, and document metadata from Creator Studio Design.`;
  if (normalizedCategory?.includes('developer')) return `Inspect ${name} developer resources, activity, and operational context from Creator Studio Design.`;
  if (normalizedCategory?.includes('crm') || normalizedCategory?.includes('sales')) return `Use ${name} customer, deal, and account context in Creator Studio Design artifacts.`;
  if (normalizedCategory?.includes('marketing')) return `Analyze ${name} campaigns, audiences, and marketing activity from Creator Studio Design.`;
  if (normalizedCategory?.includes('finance') || normalizedCategory?.includes('commerce')) return `Work with ${name} business, billing, and transaction data in Creator Studio Design.`;
  if (normalizedCategory?.includes('observability')) return `Surface ${name} incidents, metrics, and operational signals in Creator Studio Design.`;
  if (normalizedCategory?.includes('data')) return `Query ${name} datasets and platform metadata for data-backed Creator Studio Design artifacts.`;
  return `Use ${name} tools and data directly from Creator Studio Design.`;
}
