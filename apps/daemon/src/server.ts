...
// and https://github.com/mikefilsaime-groove/creator-studio-design/issues/710.
...
export function createAgentRuntimeToolPrompt(
  ...
  return [
    ...
    '- `OD_HYPERFRAMES_BIN` is the absolute path to Creator Studio Design\'s pinned HyperFrames CLI. Run lightweight commands through `OD_NODE_BIN`; use `"$OD_NODE_BIN" "$OD_BIN" media scaffold` for composition setup and never use a user-level `npx` cache.',
    '- `OD_BIN` is the absolute path to the Creator Studio Design CLI script. On POSIX shells run wrappers with `"$OD_NODE_BIN" "$OD_BIN" tools ...`; do not call bare `od`, which may resolve to the system octal-dump command on Unix-like systems.',
...
export async function startServer({
  ...
  if (apiTokenAuthEnabled) {
    ...
    app.use((req, res, next) => {
      ...
      return res.status(401).type('text/plain').send(
        'Creator Studio Design authentication required. Use username "open-design" and OD_API_TOKEN as the password.',
      );
    });
  }
  ...
  const startChatRun = async (chatBody, run) => {
    ...
    const destroyChildStdio = (child) => {
      ...
      //
      // See: https://github.com/mikefilsaime-groove/creator-studio-design/issues/4100
      if (!child) return;
    ...
    // Creator Studio Design relays them to the agent so the model can call those tools.
    ...
    if (def.id === 'codex') {
      ...
      // When Creator Studio Design leaves model selection at `default`, Codex resolves
      ...
      send(
        ...
        createSseErrorPayload(
          ...
          `Run terminated: model emitted fabricated role marker (\`${marker}\`). ` +
            ...
            'See https://github.com/mikefilsaime-groove/creator-studio-design/issues/3247.',
          { retryable: true },
        ),
      );
  ...
  orbitService.setRunHandler(async ({
    ...
    const orbitRunBody = {
      ...
      systemPrompt: [
        ...
        systemPrompt,
        'You are Orbit, an autonomous activity-summary agent inside Creator Studio Design.',
