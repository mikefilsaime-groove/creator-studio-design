...
async function materializeDevelopmentBundle(
  ...
  if (!(await fileExists(packageJsonPath))) {
    throw new AgentCompanionSetupError(
      'BUNDLED_COMPANION_INVALID',
      'This Creator Studio Design build does not contain the DeepSeek Harness connection component.',
    );
  }
...
async function stageVerifiedBundleInProfile(
  ...
  // packaged-app path such as "Creator Studio Design" at its spaces.
...
async function installDeepSeekHarnessCompanionOnce(options: {
  ...
  try {
    ...
    throw new AgentCompanionSetupError(
      'COMPANION_INSTALL_FAILED',
      'DeepSeek Harness could not install the Creator Studio Design connection component. No agent selection was changed.',
    );
  }
