...
export async function ensureMcpDaemonUrl(
  ...
  if (!explicitDaemonUrl && !registeredBootstrapTarget
    ...
    throw new Error("The Creator Studio Design MCP registration is missing its runtime connection. Open the app and refresh the MCP registration, then restart this MCP session.");
  }
  ...
  if (plan.action === "none") {
    ...
    throw new Error(
      `The registered Creator Studio Design runtime is unavailable and cannot be launched (${plan.reason}).`,
    );
  }
  ...
  throw new Error(
    `Creator Studio Design was launched headlessly but its daemon did not become ready within ${timeoutMs}ms.`,
  );
}
