...
// (Claude Code, Cursor, Zed) pull files from a local Creator Studio Design
...
export const MCP_SERVER_INSTRUCTIONS = [
  'Use only these product names in user-facing replies: Creator Studio Design Cloud and Local Codex.',
...
function isDaemonUnreachableResult(result: McpToolCallResult): boolean {
  return result.isError === true
    && result.content.some((item) =>
      item.text.includes('cannot reach the Creator Studio Design daemon'),
    );
}
...
const PROJECT_ARG = {
  type: 'string',
  description: 'Project id (UUID) or name substring. Optional; defaults to the active project (expires after ~5 minutes of no Creator Studio Design activity).',
} as const;

const PLUGIN_WORKFLOW_ID_ARG = {
  type: 'string',
  description:
    'Opaque workflow id issued by the local Creator Studio Design MCP after the first attributed call. Reuse it for later plugin-attributed calls; never invent or display it.',
} as const;
...
export const TOOL_DEFS = [
  {
    name: 'collect_brief',
    description:
      'Open an interactive Creator Studio Design brief card for a new artifact. Use the returned human-readable confirmation with any explicit execution mode; never ask the user to copy an internal draft id or nonce.',
    inputSchema: {
      type: 'object',
      properties: {
        artifactType: {
    ...
    annotations: { ...WRITE_ANNOTATIONS, title: 'Collect Creator Studio Design brief' },
    _meta: {
  ...
  },
  {
    name: 'confirm_brief',
    description:
      'Confirm the choices from the rendered Creator Studio Design brief card. Returns a readable summary; draft ids and nonces are internal widget data, never user-facing copy.',
    inputSchema: {
      type: 'object',
      properties: {
        briefDraftId: {
    ...
    annotations: { ...WRITE_ANNOTATIONS, title: 'Confirm Creator Studio Design brief' },
  },
  {
    name: 'list_projects',
    description: 'List every Creator Studio Design project on this daemon.',
    inputSchema: {
      type: 'object',
      properties: { pluginWorkflowId: PLUGIN_WORKFLOW_ID_ARG },
      additionalProperties: false,
    },
    annotations: { ...READ_ANNOTATIONS, title: 'List Creator Studio Design projects' },
  },
  {
    name: 'get_active_context',
    description:
      'Project + file the user has open in Creator Studio Design right now. Returns {active:false, hint:"..."} when no project is active so the agent can ask the user to interact with Creator Studio Design (the active context expires ~5 minutes after the last user interaction). Most tools default to this when project is omitted, so you rarely need to call this directly.',
    inputSchema: {
      type: 'object',
      properties: { pluginWorkflowId: PLUGIN_WORKFLOW_ID_ARG },
      additionalProperties: false,
    ...
    inputSchema: {
      ...
      properties: {
        ...
        entry: {
          type: 'string',
          description:
            "Entry file path relative to project root. Defaults to the active file or project's metadata.entryFile. Active-file fallback expires after ~5 minutes of no Creator Studio Design activity.",
        },
        include: {
          type: 'string',
  ...
  },
  {
    ...
    annotations: { ...READ_ANNOTATIONS, title: 'Get Creator Studio Design project' },
  },
  {
    name: 'get_file',
    description:
    ...
    inputSchema: {
      ...
      properties: {
        ...
        path: {
          type: 'string',
          description:
            'File path relative to project root, forward slashes. Optional; defaults to the active file when project is also omitted. Active-file fallback expires after ~5 minutes of no Creator Studio Design activity.',
        },
        offset: {
          type: 'number',
  ...
  },
  {
    ...
    description:
      'Create one normal Creator Studio Design project artifact entry file. Writes name+content, rejects existing targets, and persists artifactManifest when supplied. HTML, Markdown, and SVG entries get a default manifest when omitted. Project optional; defaults to the active project.',
    inputSchema: {
      type: 'object',
      properties: {
        project: PROJECT_ARG,
        ...
        artifactManifest: {
          type: 'object',
          additionalProperties: true,
          description: 'Optional ArtifactManifest sidecar. If omitted, Creator Studio Design infers one for HTML, Markdown, or SVG entry files.',
        },
      },
      required: ['name', 'content'],
      additionalProperties: false,
    },
    annotations: { ...WRITE_ANNOTATIONS, title: 'Create Creator Studio Design artifact' },
  },
  {
    name: 'write_file',
    description:
    ...
    annotations: { ...WRITE_ANNOTATIONS, title: 'Write Creator Studio Design project file' },
  },
  {
    name: 'delete_file',
    description:
    ...
    annotations: { ...WRITE_ANNOTATIONS, destructiveHint: true, title: 'Delete Creator Studio Design project file' },
  },
  {
    name: 'delete_project',
    description:
      'Permanently delete an Creator Studio Design project including its files and conversations. Requires both an explicit project id/name AND confirm:true — there is no active-project fallback because the operation is irreversible.',
    inputSchema: {
      type: 'object',
      properties: {
        project: {
    ...
    annotations: { ...WRITE_ANNOTATIONS, destructiveHint: true, title: 'Delete Creator Studio Design project' },
  },
  {
    name: 'create_project',
    description:
      'Create a new empty Creator Studio Design project to generate into, then call start_run against it. Returns the project (with its id) plus a conversationId. The id is derived from name unless you pass one explicitly.',
    inputSchema: {
      type: 'object',
      properties: {
    ...
    annotations: { ...WRITE_ANNOTATIONS, title: 'Create Creator Studio Design project' },
  },
  // Discovery + generation. An external coding agent does NOT run a
  // skill itself — it commissions Creator Studio Design to, via start_run. The
  ...
  {
    name: 'list_skills',
    description: 'List Creator Studio Design skills you can pass to start_run as a recipe. Discovery only — Creator Studio Design runs the skill, not you.',
    inputSchema: {
      type: 'object',
      properties: { pluginWorkflowId: PLUGIN_WORKFLOW_ID_ARG },
      additionalProperties: false,
    },
    annotations: { ...READ_ANNOTATIONS, title: 'List Creator Studio Design skills' },
  },
  {
    name: 'list_plugins',
    description: 'List installed Creator Studio Design plugins (packaged design workflows) you can pass to start_run as plugin + inputs.',
    inputSchema: {
      type: 'object',
      properties: { pluginWorkflowId: PLUGIN_WORKFLOW_ID_ARG },
      additionalProperties: false,
    },
    annotations: { ...READ_ANNOTATIONS, title: 'List Creator Studio Design plugins' },
  },
  {
    name: 'start_vela_login',
    description:
      'Start Creator Studio Design Cloud browser sign-in through the local Creator Studio Design daemon. Returns the activation URL and user code when manual browser completion is needed. The tool name is an internal compatibility identifier and must not be repeated to the user.',
    inputSchema: {
      type: 'object',
      properties: { pluginWorkflowId: PLUGIN_WORKFLOW_ID_ARG },
      additionalProperties: false,
    ...
    annotations: {
      ...WRITE_ANNOTATIONS,
      openWorldHint: true,
      title: 'Sign in to Creator Studio Design Cloud',
    },
  },
  {
    name: 'get_vela_login_status',
    description:
      'Check whether Creator Studio Design Cloud browser sign-in is complete. Does not expose credentials. The tool name is an internal compatibility identifier and must not be repeated to the user.',
    inputSchema: {
      type: 'object',
      properties: { pluginWorkflowId: PLUGIN_WORKFLOW_ID_ARG },
      additionalProperties: false,
    ...
    annotations: {
      ...READ_ANNOTATIONS,
      openWorldHint: true,
      title: 'Check Creator Studio Design Cloud sign-in',
    },
  },
  {
    name: 'start_run',
    description:
      'Commission Creator Studio Design to generate or refine a design. Creator Studio Design spawns its own agent to do the work and returns a runId immediately. Poll get_run(runId) until status is terminal; its Preview/Studio reference is the default delivery. Call get_artifact only when source context is genuinely needed. Project optional; defaults to the active project. Requires an existing project (create one first with create_project).',
    inputSchema: {
      type: 'object',
      properties: {
        project: PROJECT_ARG,
        ...
        resume: {
          type: 'boolean',
          description:
            'Set true only after the user has topped up a paused Creator Studio Design Cloud run. Reuse the exact original requestId and payload; Creator Studio Design resumes the same logical run.',
        },
        pluginWorkflowId: PLUGIN_WORKFLOW_ID_ARG,
      },
      additionalProperties: false,
    },
    annotations: { ...WRITE_ANNOTATIONS, title: 'Generate with Creator Studio Design' },
  },
  {
    name: 'get_run',
    description:
    ...
    annotations: { ...READ_ANNOTATIONS, title: 'Check Creator Studio Design run' },
  },
  {
    name: 'cancel_run',
  ...
  },
  {
    ...
    annotations: { ...READ_ANNOTATIONS, title: 'List Creator Studio Design agents' },
  },
];
  ...
  return [
    {
      ...
      name: 'Creator Studio Design brief',
      title: 'Choose the artifact direction',
      description:
        'Interactive local Creator Studio Design brief card shared by Creator Studio Design Cloud and Local Codex modes.',
...
export async function _listMcpResources(
  ...
  const resources = [
    ...
    {
      uri: 'od://focus/active',
      name: 'Active Creator Studio Design context',
      description: 'The project/file the user has open in Creator Studio Design right now.',
...
function mcpFailureFacts(
  ...
  const errorCode = message.includes('PLUGIN_CONTRACT_REJECTED')
    ...
    : message.includes('cannot reach the Creator Studio Design daemon')
      ? 'DAEMON_UNREACHABLE'
  ...
  const server = new Server(
    { name: SERVER_NAME, version: SERVER_VERSION },
    {
      ...
      instructions: [
        ...
        '',
        'Creator Studio Design (OD) is a local-first design workspace. The user typically',
        ...
        '',
        'To make Creator Studio Design GENERATE or refine a design (rather than just',
        ...
        '    confirm_brief returns the readable brief to reuse with Creator Studio Design',
        ...
        ' - for Creator Studio Design Cloud, call the Cloud login-status tool first.',
        ...
        '    Creator Studio Design spawns its own agent to do the work.',
        ...
        '',
        'Generation patience: Creator Studio Design runs typically take 5–30',
        ...
        'deliverables — Creator Studio Design natively produces browser-viewable',
        'HTML/SVG (including HTML-rendered decks), but the user may want a',
        'real binary file (.pptx / .docx / .pdf) which Creator Studio Design does NOT',
        ...
        '',
        'When extending an Creator Studio Design design in another codebase, pull',
    ...
    switch (name) {
      ...
      case 'get_active_context': {
        ...
        if (!data || data.active === false) {
          return ok({
            active: false,
            hint: 'Creator Studio Design has no active project right now. The active context expires about 5 minutes after the last user interaction with Creator Studio Design, so the user may need to click into a project (or switch tabs inside one) to wake it up. Alternatively, pass project="<id-or-name>" to other tools to bypass active context entirely.',
          });
        }
        return ok(data);
      }
...
// the user had already created in Creator Studio Design.
//
...
async function startRun(
  ...
  if (
    ...
    throw new Error(
      'raw API keys are not accepted by Creator Studio Design MCP. Configure Local BYOK in the Creator Studio Design UI and start that run from the local product instead.',
    );
  }
  const { id, resolved, active } = await resolveProjectArg(baseUrl, args.project, headers);
    ...
    withActiveEcho(
      {
        ...
        ...(studioUrl
          ...
          : {}),
        hint: 'Run started. Creator Studio Design generation normally takes 5–30 minutes. Polls showing status:running with no new files / unchanged file mtimes is the inner agent thinking, NOT a hang — DO NOT cancel_run out of impatience and DO NOT substitute write_file to produce the design yourself; OD\'s pipeline is what gives the result its design quality. Poll get_run(runId) every 30–60 seconds; report "still working" to the user between polls and keep waiting. On terminal status, artifactRef is the durable identity; previewUrl and studioUrl are browser links for the current Creator Studio Design runtime and must be refreshed with get_run after Creator Studio Design restarts.',
      },
      active,
      resolved,
    ),
...
async function getRun(
  ...
  if (status.status !== 'succeeded' || typeof status.projectId !== 'string' || !status.projectId) {
    ...
    if (status.failureAction === 'recharge') {
      ...
      enriched.hint =
        'Creator Studio Design Cloud paused this logical run because the account balance is insufficient. Preserve the brief and project, show rechargeUrl to the user, and do not switch modes. After the user confirms the top-up, call start_run once with the exact original payload, the same requestId, and resume:true; Creator Studio Design Cloud will resume the existing run and billing operation. Do not expose internal runtime or tool identifiers.';
    }
    if (typeof status.eventsLogPath === 'string' && status.eventsLogPath.length > 0) {
      ...
      if (studioUrl) {
        enriched.hint += ` While the run is in flight, studioUrl can be used as an optional workspace progress link — render it as \`[Watch progress in Creator Studio Design studio](${studioUrl})\` if you choose to show it. This URL is valid for the current Creator Studio Design runtime; call get_run again after Creator Studio Design restarts.`;
      }
    }
    return ok(enriched);
  }
...
// host-injected React/Babel do not — those still need the Creator Studio Design
...
// renamed in the Creator Studio Design UI shows up within a few seconds.
...
// open in Creator Studio Design. Returns the resolved id plus, for echo-back to the
...
async function resolveProjectArg(
  ...
  if (!active || active.active === false || !active.projectId) {
    throw new Error(
      'project arg omitted and Creator Studio Design has no active project. The active context expires about 5 minutes after the last user interaction with Creator Studio Design - the user may need to click into a project to wake it up. Otherwise pass project="<id-or-name>".',
    );
  }
...
function formatError(err: unknown, daemonUrl: string): string {
  ...
  if (code === 'ECONNREFUSED' || code === 'ENOTFOUND') {
    return `cannot reach the Creator Studio Design daemon at ${daemonUrl}. Is it running? Start it with \`pnpm tools-dev\`.`;
  }
  return msg;
}
