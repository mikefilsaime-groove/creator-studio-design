...
export function registerMcpRoutes(app: Express, ctx: RegisterMcpRoutesDeps) {
  ...
  function computeInstallPayload(): McpInstallPayload {
    ...
    // knows where the browser-facing Creator Studio Design studio is running.
  ...
  // External MCP server configuration. Creator Studio Design connects to these as a
...
function renderOAuthResultPage(opts: any) {
  ...
  const body = ok
    ? `Your MCP server <code>${escapeHtml(opts.serverId ?? '')}</code> is now connected. You can close this tab and return to Creator Studio Design.`
...
<head>
<meta charset="utf-8" />
<title>${escapeHtml(title)} — Creator Studio Design</title>
