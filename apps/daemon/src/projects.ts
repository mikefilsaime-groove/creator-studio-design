...
function buildDesignManifest(entries, projectLabel) {
  ...
  return JSON.stringify({
    ...
    title: projectLabel || 'Creator Studio Design project',
    entryFile,
    sourceFiles: {
      all: files,
      html: htmlFiles,
...
function buildDesignHandoff(entries, projectLabel) {
  ...
  return `# ${projectLabel || 'Creator Studio Design project'} implementation handoff
...
- Keep generated product UI free of Creator Studio Design chrome, preview labels, or design-process annotations.
...
- Do not keep prototype-only annotations, frame labels, or Creator Studio Design chrome in the production UI.
