...
export const PLUGIN_SHARE_ACTION_LABELS: Record<PluginShareAction, string> = {
  ...
  'contribute-open-design': 'Contribute to Creator Studio Design',
};
...
export function renderPluginSharePrompt({ action, sourcePlugin, stagedPath }: PluginSharePromptInput): string {
  ...
  if (action === 'publish-github') {
    return [
      `Publish the local Creator Studio Design plugin "${title}" as a new public GitHub repository.`,
      '',
      `The plugin source files have been copied into this project at \`${stagedPath}\`.`,
      'Use the local daemon share endpoint so the publish flow runs through Creator Studio Design\'s validated GitHub path:',
      '',
      '```bash',
  ...
  return [
    `Open a pull request to add the local Creator Studio Design plugin "${title}" to the Creator Studio Design repository.`,
    '',
    `The plugin source files have been copied into this project at \`${stagedPath}\`.`,
    'Use the local daemon share endpoint so the contribution flow runs through Creator Studio Design\'s validated GitHub path:',
    '',
    '```bash',
...
export function upsertSkillPluginCandidateAssistantMessage(
  ...
  upsertMessage(db, run.conversationId, {
    id: messageId,
    role: 'assistant',
    content: `Creator Studio Design found reusable skill material that can become a plugin: ${candidate.title}`,
    events: [{
