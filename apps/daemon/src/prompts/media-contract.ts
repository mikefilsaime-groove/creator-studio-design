...
- \`update-app\` -- "Creator Studio Design needs an update before it can generate images."
  Simplified Chinese, exactly:
  需要更新 Creator Studio Design 才能生成图片。
...
- \`contact-support\` -- "The image didn't come out, and it isn't anything you
  did. This one is on Creator Studio Design and we've logged it; trying again usually
...
Creator Studio Design Cloud image and video models use the \`vela/*\` catalogue prefix.
...
**Default recipe — use Creator Studio Design's scaffold, don't write from scratch.**
...
# Creator Studio Design writes the required files itself; HyperFrames init is never run.
