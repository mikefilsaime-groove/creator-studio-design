...
### Creator Studio Design Cloud media defaults
...
This run uses Creator Studio Design's filesystem execution profile. Project files are the source of truth for generated artifacts.
...
// to a "writable roots" setting Creator Studio Design does not have. The user searched
...
Creator Studio Design does not currently expose a sandbox mode, a writable-directory or "writable roots" list, or an approval-policy setting. Do not tell the user to look for one, and do not invent a settings path, menu, or option name to explain the failure — they will go looking and find nothing. Describe the limitation plainly and point at the Skills tab instead. (If a future build does add such a surface, this paragraph is what should change.)`;
...
For mid-conversation clarification you may still emit a \`<question-form>\` block — it is markup the Creator Studio Design UI parses, not a native tool call.`;

const PLAN_MODE_OVERRIDE = `# Plan mode — editable document first (read first — overrides every rule below)

This conversation is in Creator Studio Design Plan mode. Use the same context, files, attachments, connectors, MCP servers, project memory, tools, and design systems as Design mode, but do NOT create the final design artifact first.
...
function renderMediaMetadataAction(
  ...
  if (mode === 'disabled') {
    return `This is ${article} **${surface}** project, but Creator Studio Design-owned media execution is disabled for this run. Plan the creative brief only unless an external MCP media tool is explicitly configured. Do NOT call OD media generation tools and do NOT emit \`<artifact>\` HTML for media surfaces.`;
  }
