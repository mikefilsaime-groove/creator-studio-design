...
export function diagnoseClaudeCliFailure(
  ...
  if (authFailure) {
    ...
    const configHint = hasConfigDir
      ...
      : 'If you use multiple Claude profiles, set CLAUDE_CONFIG_DIR in Settings so Creator Studio Design spawns the same profile that works in your terminal.';
    return withContext(
      'Claude Code could not authenticate. Run `claude`, use `/login`, then retry the Creator Studio Design request.',
  ...
  if (windowsCredentialMismatch) {
    return withContext(
      ...
      'Re-authenticate Claude Code in the same Windows, WSL, or shell environment that Creator Studio Design uses. On native Windows, check Windows Credential Manager if `/login` does not repair the session.',
      input,
    );
  }
  ...
  if (configStateFailure) {
    ...
    const detail = hasConfigDir
      ? 'Re-run `claude` and `/login` for that profile, then retry Creator Studio Design.'
  ...
  if (!text.trim() && input.exitCode === 1) {
    ...
    const detail = hasConfigDir
      ? 'Re-run `claude` and `/login` for that profile, then retry Creator Studio Design.'
      : 'Run `claude`, use `/login`, and retry. If you use multiple Claude profiles, set CLAUDE_CONFIG_DIR in Settings so Creator Studio Design uses the same profile as your terminal.';
    return withContext(
      message,
      detail,
      input,
