...
// and attributes it to the Creator Studio Design open-source project for shareability.
export function buildDesignSystemSkillsMarkdown(input: {
  ...
  lines.push('');
  lines.push(
    'Generated with **Creator Studio Design** — the open-source, local-first Claude Design alternative. ' +
...
async function migrateLegacyDesignSystemPackage(
  ...
  const title = normalizeTitle(metadata.title ?? firstHeading(body) ?? id);
  const summary = summarize(body) || 'A reusable Creator Studio Design design system.';
  ...
  await Promise.all([
    ...
    appKitExists
      ? writeIfMissing(
          ...
          `# ${title} UI Kit\n\nThis package was migrated from an earlier Creator Studio Design design-system workspace. Use \`index.html\` as the applied interface example and replace it with source-backed modular components when new repository evidence is available.\n`,
        )
...
function generatedDesignSystemFileWrites(
  ...
  const summary = input.summary || 'A user-created Creator Studio Design design system.';
...
function buildDraftDesignSystemBody(input: UserDesignSystemInput & { title: string }): string {
  ...
  const summary = cleanText(input.summary) || 'A user-authored design system for future Creator Studio Design projects.';
...
A reusable Creator Studio Design package for ${input.title}.
...
function renderSkill(input: {
  ...
  return `---
name: ${skillName}
description: Use this skill when generating Creator Studio Design artifacts that should follow ${input.title}.
user-invocable: true
---
...
function renderOverviewHtml(
  ...
  return renderHtmlDocument(
    title,
    `<main class="overview">
      <p class="eyebrow">Creator Studio Design system</p>
