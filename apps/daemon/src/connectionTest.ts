...
function codexExecutableGuidance(
  ...
  return ` Configured Codex path failed: ${configuredOverridePath}. Creator Studio Design also detected a PATH Codex CLI at ${pathResolvedPath}. Update CODEX_BIN or clear the custom path to use the detected binary.`;
}
...
async function prepareOpenCodeConnectionTestCwd(tempDir: string): Promise<void> {
  await fsp.writeFile(
    ...
    'Creator Studio Design OpenCode connection test.\n',
    'utf8',
  );
  try {
