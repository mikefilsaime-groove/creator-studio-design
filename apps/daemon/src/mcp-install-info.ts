...
export interface BuildMcpInstallPayloadInputs {
  ...
  /** Browser-facing Creator Studio Design studio base URL (e.g.
...
export function buildMcpInstallPayload(
  ...
  if (!inputs.cliExists) {
    hints.push(
      `Creator Studio Design CLI entry is missing at ${inputs.cliPath}. Rebuild the daemon or packaged app and refresh.`,
    );
  }
  if (!inputs.nodeExists) {
    hints.push(
      `Node-compatible runtime at ${inputs.execPath} no longer exists. Reinstall Creator Studio Design or Node and restart the daemon.`,
    );
  }
