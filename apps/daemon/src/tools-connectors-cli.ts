...
Environment:
  ...
  OD_BIN          Creator Studio Design CLI script for agent wrapper invocations
...
export async function auditDesignSystemPackage(
  ...
  if (options.referencePackage === true) {
    if (!fileSet.has('DESIGN.md')) {
      addIssue('warning', 'missing_open_design_rules', 'Reference packages may omit DESIGN.md, but generated Creator Studio Design packages must include it as the canonical rules file.', 'DESIGN.md');
    }
  } else {
