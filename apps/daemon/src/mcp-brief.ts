...
const BRIEF_COPY: Record<LocalMcpBriefLocale, LocalizedBriefCopy> = {
  en: {
    ...
    description:
      'Choose one option for each decision. The same readable brief can be used with Creator Studio Design Cloud or Local Codex.',
    submitLabel: 'Confirm brief',
    completeCard:
      'Complete the rendered Creator Studio Design brief card. The confirmation returns a readable summary; internal correlation values must remain hidden.',
  ...
  'zh-CN': {
    ...
    description:
      '请为每项决策选择一个选项。确认后的可读需求可用于 Creator Studio Design Cloud、本地 Codex 或安全 BYOK。',
    submitLabel: '确认需求',
    completeCard: '请填写显示的 Creator Studio Design 需求卡。确认后会返回可读摘要；不要向用户展示内部关联值。',
    confirmed: '需求已确认。',
  ...
  'zh-TW': {
    ...
    description:
      '請為每項決策選擇一個選項。確認後的可讀需求可用於 Creator Studio Design Cloud、本機 Codex 或安全 BYOK。',
    submitLabel: '確認需求',
    completeCard: '請填寫顯示的 Creator Studio Design 需求卡。確認後會回傳可讀摘要；不要向使用者顯示內部關聯值。',
    confirmed: '需求已確認。',
  ...
  ja: {
    ...
    description:
      '各項目から 1 つ選択してください。確認したブリーフは Creator Studio Design Cloud、ローカル Codex、安全な BYOK で利用できます。',
    submitLabel: 'ブリーフを確認',
    completeCard:
      '表示された Creator Studio Design のブリーフカードに回答してください。確認後は読みやすい概要が返されます。内部の関連付け値は表示しないでください。',
...
export function createLocalMcpBriefStore(
  ...
  return {
    ...
    confirm(input) {
      ...
      if (!draft) {
        throw new Error(
          'The Creator Studio Design brief has expired or is unknown. Call collect_brief again.',
        );
      }
      if (draft.nonce !== nonce) {
        throw new Error('The Creator Studio Design brief nonce is invalid.');
      }
      ...
      if (!decision.complete) {
        ...
        throw new Error(
          `The Creator Studio Design brief is incomplete. Missing: ${missing.join(', ')}.`,
        );
      }
      ...
      if (draft.confirmation) {
        if (draft.confirmationAnswersDigest !== confirmationAnswersDigest) {
          throw new Error(
            'This Creator Studio Design brief was already confirmed with different answers.',
          );
        }
...
function readProjectTitle(value: unknown): string {
  if (value === undefined) return 'Untitled Creator Studio Design artifact';
