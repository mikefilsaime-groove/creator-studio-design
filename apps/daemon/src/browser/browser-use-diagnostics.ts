...
export function isBrowserUseRequested(...values: unknown[]): boolean {
  return values.some((value) => (
    ...
    (
      ...
      value.includes('Use the selected Creator Studio Design Browser tab as the bound target.')
    )
  ));
}
...
export function renderBrowserUseUnavailablePrompt(state: BrowserUseRunState | null): string {
  ...
  return [
    ...
    '',
    `Browser automation was requested, but Creator Studio Design has not confirmed a matching in-app browser backend for this run. Reason: \`${state.reason}\`.`,
