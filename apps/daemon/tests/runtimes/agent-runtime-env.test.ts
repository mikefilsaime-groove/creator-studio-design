...
describe('agent runtime tool environment', () => {
  it('prefers explicit OD_NODE_BIN over the process executable', () => {
    expect(resolveOpenDesignNodeBin({
      env: { OD_NODE_BIN: 'C:\\Creator Studio Design\\resources\\open-design\\bin\\node.exe' },
      execPath: 'C:\\Users\\Ada\\AppData\\Roaming\\Creator Studio Design\\en\\hash\\Creator Studio Design.exe',
      platform: 'win32',
      resourceRoot: null,
    })).toBe('C:\\Creator Studio Design\\resources\\open-design\\bin\\node.exe');
  });
  ...
  it('resolves the bundled resource node before falling back to process.execPath', () => {
    expect(resolveOpenDesignNodeBin({
      ...
      execPath: 'C:\\Users\\Ada\\AppData\\Roaming\\Creator Studio Design\\en\\hash\\Creator Studio Design.exe',
      platform: 'win32',
      resourceRoot: 'C:\\Users\\Ada\\AppData\\Local\\Programs\\Creator Studio Design\\resources\\open-design',
      exists: (candidate) => candidate.endsWith('\\resources\\open-design\\bin\\node.exe'),
    })).toBe('C:\\Users\\Ada\\AppData\\Local\\Programs\\Creator Studio Design\\resources\\open-design\\bin\\node.exe');
  });
