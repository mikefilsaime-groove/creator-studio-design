...
describe('local MCP plugin observability contract', () => {
  ...
  it('accepts the bounded Creator Studio Design context and rejects extra or secret fields', () => {
  ...
  it('keeps MCP transport failures and delivery completeness as separate facts', () => {
    expect(mcpFailureFacts('start_run', {
      isError: true,
      content: [{ type: 'text', text: 'cannot reach the Creator Studio Design daemon' }],
    })).toEqual({
