...
process.stdin.on('end', () => {
  ...
  console.log(JSON.stringify({ type: 'step_start' }));
  console.log(JSON.stringify({ type: 'text', part: { text: '我来帮你创建一个通用的 Creator Studio Design 插件脚手架。先读取文档规范，再生成插件文件。' } }));
  console.log(JSON.stringify({ type: 'step_finish', part: { tokens: { input: 1, output: 1 } } }));
  process.exit(0);
});
`,
      async () => {
        const createResponse = await fetch(`${baseUrl}/api/runs`, {
          ...
          body: JSON.stringify({
            ...
            message: '请创建一个可刷新、可审计、由 API 驱动的 Creator Studio Design 插件脚手架。',
          }),
        });
        expect(createResponse.status).toBe(202);
        const { runId } = await createResponse.json() as { runId: string };
...
`,
      async () => {
        const response = await fetch(`${baseUrl}/api/chat`, {
          ...
          body: JSON.stringify({
            agentId: 'opencode',
            message: 'build the Creator Studio Design landing page',
