...
describe('userFacingAgentLabel', () => {
  it('prefers the configured agent id over the resolved executable path', () => {
    expect(
      userFacingAgentLabel(
        'claude',
        '/Applications/Creator Studio Design Beta.app/Contents/Resources/open-design/bin/claude',
      ),
    ).toBe('claude');
  });
  ...
  it('falls back to the executable basename when agent id is missing', () => {
    expect(
      userFacingAgentLabel(
        null,
        '/Applications/Creator Studio Design Beta.app/Contents/Resources/open-design/bin/vela',
      ),
    ).toBe('vela');
  });
  ...
  it('strips Windows executable extensions from basename fallbacks', () => {
    expect(
      userFacingAgentLabel(
        '',
        'C:\\Program Files\\Creator Studio Design\\resources\\open-design\\bin\\unknown.exe',
      ),
    ).toBe('unknown');
  });
