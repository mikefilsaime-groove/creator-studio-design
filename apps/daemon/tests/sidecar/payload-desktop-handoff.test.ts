...
describe("legacy payload desktop handoff", () => {
  ...
  it("captures the real previous pointer before old outer confirm, then arms and launches payload desktop", async () => {
    ...
    try {
      ...
      const outerExecutablePath = join(root, "installed", "Creator Studio Design Beta.app", "Contents", "MacOS", "Creator Studio Design Beta");
      ...
      await writeFile(launcherPaths.installPath, `${JSON.stringify({
        channel: "beta",
        launchPath: join(root, "installed", "Creator Studio Design Beta.app"),
        namespace,
        schemaVersion: LAUNCHER_SCHEMA_VERSION,
      })}\n`);
  ...
  it("resolves the installed outer and payload executable for the real Windows beta namespace", async () => {
    ...
    try {
      ...
      const outerExecutablePath = join(root, "installed", "Creator Studio Design Beta.exe");
      const payloadExecutablePath = join(versionPaths.payloadRoot, "Creator Studio Design Beta.exe");
      ...
      await writeFile(versionPaths.manifestPath, `${JSON.stringify({
        ...
        entry: {
          cwd: "payload",
          executable: "payload/Creator Studio Design Beta.exe",
        },
        namespace,
        payloadRoot: "payload",
        platform: "win32",
