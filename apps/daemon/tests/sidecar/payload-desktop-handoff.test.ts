...
async function fixture() {
  ...
  const outerBundlePath = join(root, "installed", "Creator Studio Design Beta.local.app");
  const outerExecutablePath = join(outerBundlePath, "Contents", "MacOS", "Creator Studio Design Beta");
  const payloadExecutablePath = join(versionPaths.payloadRoot, "Creator Studio Design Beta.app", "Contents", "MacOS", "Creator Studio Design Beta");
  ...
  await writeFile(versionPaths.manifestPath, JSON.stringify({
    channel: "beta",
    entry: { executable: "payload/Creator Studio Design Beta.app/Contents/MacOS/Creator Studio Design Beta" },
    namespace,
    platform: "darwin",
  ...
  it("recognizes the installed outer through a symlinked launcher root", async () => {
    ...
    try {
      ...
      await writeFile(value.launcherPaths.installPath, JSON.stringify({
        channel: "beta",
        launchPath: join(aliasRoot, "installed", "Creator Studio Design Beta.local.app"),
        namespace: value.namespace,
        schemaVersion: LAUNCHER_SCHEMA_VERSION,
      }));
