...
async function writeLauncherPayloadFixture(destinationRoot: string, version: string): Promise<void> {
  ...
  await writeFile(join(destinationRoot, "payload", "Creator Studio Design.exe"), "");
  ...
  await writeFile(join(destinationRoot, "manifest.json"), `${JSON.stringify({
    ...
    entry: {
      cwd: "payload",
      executable: "payload/Creator Studio Design.exe",
    },
...
describe("desktop updater", () => {
  ...
  it("falls back to the installer when launcher context is valid but metadata has no payload artifact", async () => {
    ...
    const launcherLaunchPath = join(root, "installed", "Creator Studio Design Beta.exe");
    try {
  ...
  it("downloads and applies launcher payload only when launcher runtime context validates", async () => {
    ...
    try {
      ...
      const updater = createDesktopUpdater({
        ...
        processExecPath: "C:\\Program Files\\Creator Studio Design Beta\\Creator Studio Design Beta.exe",
        processPid: 4242,
      });
    ...
    const updaterDeps: NonNullable<Parameters<typeof createDesktopUpdater>[1]> = {
      extractLauncherPayloadArchive: async ({ destinationRoot }) => {
        ...
        await writeFile(
          ...
          `${JSON.stringify({
            channel: "beta",
            entry: { cwd: "payload", executable: "payload/Creator Studio Design.exe" },
            namespace: "release-beta-win",
            payloadRoot: "payload",
            platform: "win32",
            schemaVersion: LAUNCHER_SCHEMA_VERSION,
  ...
  it("keeps using the installer when launcher context has a missing installed launch path", async () => {
    ...
    const launcherRuntimePath = join(root, "launcher", "runtime.json");
    const launcherLaunchPath = join(root, "missing", "Creator Studio Design Beta.exe");
    try {
      await mkdir(join(root, "launcher"), { recursive: true });
      ...
      const updater = createDesktopUpdater({
        ...
        processExecPath: "C:\\Users\\runneradmin\\AppData\\Roaming\\Creator Studio Design Beta\\launcher\\channels\\beta\\namespaces\\release-beta-win\\versions\\1.0.0-beta.1\\payload\\Creator Studio Design.exe",
      });
  ...
  it("relaunches mac launcher payloads through the prepared payload executable", async () => {
    ...
    const launcherRuntimePath = join(root, "launcher", "runtime.json");
    const launcherRoot = root;
    const launcherLaunchPath = join(root, "installed", "Creator Studio Design Beta.app");
    const launches: Array<{ appPid: number; launchPath: string; root: string }> = [];
    try {
      ...
      const updater = createDesktopUpdater({
        ...
        extractLauncherPayloadArchive: async ({ destinationRoot }) => {
          await mkdir(join(destinationRoot, "payload", "Creator Studio Design Beta.app", "Contents", "MacOS"), { recursive: true });
          await mkdir(join(destinationRoot, "payload", "Creator Studio Design Beta.app", "Contents", "Resources", "open-design"), { recursive: true });
          await writeFile(join(destinationRoot, "payload", "Creator Studio Design Beta.app", "Contents", "MacOS", "Creator Studio Design Beta"), "");
          await writeFile(join(destinationRoot, "payload", "Creator Studio Design Beta.app", "Contents", "Resources", "open-design-config.json"), "{}\n");
          await writeFile(
            join(destinationRoot, "manifest.json"),
            `${JSON.stringify({
              channel: "beta",
              entry: {
                cwd: "payload/Creator Studio Design Beta.app",
                executable: "payload/Creator Studio Design Beta.app/Contents/MacOS/Creator Studio Design Beta",
              },
              namespace: "release-beta",
              payloadRoot: "payload",
              platform: "darwin",
        ...
        processExecPath: join(root, "launcher", "channels", "beta", "namespaces", "release-beta", "versions", "1.0.0-beta.2", "payload", "Creator Studio Design Beta.app", "Contents", "MacOS", "Creator Studio Design Beta"),
        processPid: 4243,
      });
      ...
      expect(launches).toEqual([
        {
          ...
          launchPath: join(
            ...
            "1.0.0-beta.3",
            "payload",
            "Creator Studio Design Beta.app",
            "Contents",
            "MacOS",
  ...
  it("relaunches prerelease mac launcher payloads through the prerelease app bundle", async () => {
    ...
    const launcherRuntimePath = join(root, "launcher", "runtime.json");
    const launcherRoot = root;
    const launcherLaunchPath = join(root, "installed", "Creator Studio Design Prerelease.app");
    const launches: Array<{ appPid: number; launchPath: string; root: string }> = [];
    try {
      await mkdir(launcherLaunchPath, { recursive: true });
      await mkdir(join(root, "launcher"), { recursive: true });
      ...
      const updater = createDesktopUpdater({
        ...
        extractLauncherPayloadArchive: async ({ destinationRoot }) => {
          await mkdir(join(destinationRoot, "payload", "Creator Studio Design Prerelease.app", "Contents", "MacOS"), { recursive: true });
          await mkdir(join(destinationRoot, "payload", "Creator Studio Design Prerelease.app", "Contents", "Resources", "open-design"), { recursive: true });
          await writeFile(join(destinationRoot, "payload", "Creator Studio Design Prerelease.app", "Contents", "MacOS", "Creator Studio Design Prerelease"), "");
          await writeFile(join(destinationRoot, "payload", "Creator Studio Design Prerelease.app", "Contents", "Resources", "open-design-config.json"), "{}\n");
          await writeFile(
            join(destinationRoot, "manifest.json"),
            `${JSON.stringify({
              channel: "prerelease",
              entry: {
                cwd: "payload/Creator Studio Design Prerelease.app",
                executable: "payload/Creator Studio Design Prerelease.app/Contents/MacOS/Creator Studio Design Prerelease",
              },
              namespace: "release-prerelease",
              payloadRoot: "payload",
              platform: "darwin",
        ...
        processExecPath: join(root, "launcher", "channels", "prerelease", "namespaces", "release-prerelease", "versions", "1.0.0-prerelease.2", "payload", "Creator Studio Design Prerelease.app", "Contents", "MacOS", "Creator Studio Design Prerelease"),
        processPid: 4244,
      });
      ...
      const payloadLaunchPath = join(
        ...
        "1.0.0-prerelease.3",
        "payload",
        "Creator Studio Design Prerelease.app",
        "Contents",
        "MacOS",
        "Creator Studio Design Prerelease",
      );
  ...
  it("relaunches Windows launcher payloads through the prepared payload executable", async () => {
    ...
    const launcherRuntimePath = join(root, "launcher", "runtime.json");
    const launcherRoot = root;
    const launcherLaunchPath = join(root, "installed", "Creator Studio Design.exe");
    const launches: Array<{ appPid: number; launchPath: string; root: string }> = [];
    try {
      await mkdir(join(root, "installed"), { recursive: true });
      await writeFile(launcherLaunchPath, "");
      ...
      const updater = createDesktopUpdater({
        ...
        processExecPath: "C:\\Users\\runneradmin\\AppData\\Roaming\\Creator Studio Design Beta\\launcher\\channels\\beta\\namespaces\\release-beta-win\\versions\\1.0.0-beta.2\\payload\\Creator Studio Design.exe",
        processPid: 4244,
      });
