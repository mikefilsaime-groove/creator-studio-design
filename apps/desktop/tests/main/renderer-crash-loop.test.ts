...
describe("renderer crash-loop breaker wiring", () => {
  ...
  test("the crash screen offers report + save-logs + email actions via already-exposed IPC", () => {
    ...
    expect(runtimeSource).toContain("support@clickcampaigns.ai");
...
describe("isSupportMailtoUrl", () => {
  test("allows a mailto to the support address carrying only subject/body", () => {
    expect(isSupportMailtoUrl("mailto:support@clickcampaigns.ai")).toBe(true);
    expect(isSupportMailtoUrl("mailto:support@clickcampaigns.ai?subject=Crash&body=hi")).toBe(true);
  ...
  test("rejects extra recipients/headers smuggled through the query (to/cc/bcc/unknown)", () => {
    ...
    expect(isSupportMailtoUrl("mailto:support@clickcampaigns.ai?bcc=attacker@example.com")).toBe(false);
    expect(isSupportMailtoUrl("mailto:support@clickcampaigns.ai?cc=attacker@example.com")).toBe(false);
    expect(isSupportMailtoUrl("mailto:support@clickcampaigns.ai?to=attacker@example.com")).toBe(false);
    expect(isSupportMailtoUrl("mailto:support@clickcampaigns.ai?subject=x&bcc=attacker@example.com")).toBe(false);
    expect(isSupportMailtoUrl("mailto:support@clickcampaigns.ai?whatever=1")).toBe(false);
  });
  ...
  test("rejects a CR/LF-injected subject/body that could smuggle a mail header", () => {
    ...
    expect(
      isSupportMailtoUrl("mailto:support@clickcampaigns.ai?subject=ok%0D%0ABcc:attacker@example.com"),
    ).toBe(false);
    expect(isSupportMailtoUrl("mailto:support@clickcampaigns.ai?body=line1%0Aline2")).toBe(false);
  });
});
...
describe("isFirstPartyMailtoUrl", () => {
  test("covers every address the app itself offers to email", () => {
    ...
    expect(isFirstPartyMailtoUrl("mailto:support@clickcampaigns.ai")).toBe(true);
  });
