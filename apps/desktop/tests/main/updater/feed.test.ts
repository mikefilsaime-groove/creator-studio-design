...
describe("desktop updater feed", () => {
  it("resolves the installed outer version from the platform bundle layout", async () => {
    const root = makeRoot();
    try {
      const appRoot = join(root, "Creator Studio Design.app");
      ...
      const winExe = join(root, "win-install", "Creator Studio Design Beta.exe");
      ...
      const malformedExe = join(root, "broken-install", "Creator Studio Design.exe");
