...
async function writeActiveMacPayloadFixture(
  ...
  const appRoot = join(versionPaths.payloadRoot, "Creator Studio Design Beta.app");
  const resourcesPath = join(appRoot, "Contents", "Resources");
  await mkdir(join(appRoot, "Contents", "MacOS"), { recursive: true });
  await mkdir(resourcesPath, { recursive: true });
  await writeFile(join(appRoot, "Contents", "MacOS", "Creator Studio Design Beta"), "");
  await writeFile(
...
describe("resolvePackagedLauncherRuntime", () => {
  ...
  it("uses the active launcher payload when runtime state and payload manifest are valid", async () => {
    ...
    try {
      ...
      const resourcesPath = join(versionPaths.payloadRoot, "Creator Studio Design Beta.app", "Contents", "Resources");
      ...
      await mkdir(join(versionPaths.payloadRoot, "Creator Studio Design Beta.app", "Contents", "MacOS"), { recursive: true });
      ...
      await writeFile(
        ...
        `${JSON.stringify({
          channel: "beta",
          launchPath: "/Applications/Creator Studio Design Beta.app",
          namespace: config.namespace,
          schemaVersion: LAUNCHER_SCHEMA_VERSION,
        })}\n`,
      );
      ...
      const runtime = await resolvePackagedLauncherRuntime(config, paths, {
        ...
        currentExecutablePath: "/Applications/Creator Studio Design Beta.app",
      });

      expect(runtime.source).toBe("payload");
      expect(runtime.desktopExecutablePath).toBe(payloadExecutablePath);
      expect(runtime.electronNodeCommand).toBeNull();
      expect(runtime.installedLaunchPath).toBe("/Applications/Creator Studio Design Beta.app");
  ...
  it("refreshes install.json launchPath to the current launcher executable when the persisted path is stale", async () => {
    ...
    try {
      ...
      await writeFile(
        ...
        `${JSON.stringify({
          channel: "beta",
          launchPath: "/Applications/Creator Studio Design Legacy.app",
          namespace: config.namespace,
          schemaVersion: LAUNCHER_SCHEMA_VERSION,
        })}\n`,
      );
  ...
  it("uses the Windows payload executable as the Electron-as-Node command when payload Node is absent", async () => {
    ...
    try {
      ...
      const payloadExePath = join(versionPaths.versionRoot, "payload", "Creator Studio Design.exe");
