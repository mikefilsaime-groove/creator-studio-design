...
function fakeRuntime(payloadDesktopProcess: boolean): PackagedLauncherRuntime {
  return {
    ...
    desktopExecutablePath: "/tmp/payload/Creator Studio Design Beta.app/Contents/MacOS/Creator Studio Design Beta",
    descriptor: {} as PackagedLauncherRuntime["descriptor"],
    electronNodeCommand: null,
    installedLaunchPath: "/Applications/Creator Studio Design Beta.app",
...
describe("payload desktop delegation", () => {
  ...
  it("forwards only the OS invite URL across an outer-to-payload cold start", () => {
    ...
    expect(findPackagedDeeplinkArg(["Creator Studio Design.exe", "--unrelated", deeplink])).toBe(deeplink);
    expect(findPackagedDeeplinkArg(["Creator Studio Design.exe", "--unrelated"])).toBeNull();
    const plan = planPackagedPayloadDesktopDelegation(fakeRuntime(false), stamp, {
      currentPid: 4321,
      forwardedArgs: ["Creator Studio Design.exe", "--unrelated", deeplink],
      timeoutMs: 60_000,
    });
