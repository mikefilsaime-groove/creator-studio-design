...
const ISSUE_4638_LOG = `Error [ERR_MODULE_NOT_FOUND]: Cannot find package 'better-sqlite3' imported from /Applications/Creator Studio Design.app/Contents/Resources/app/prebundled/daemon/chunks/server-PULTSXNL.mjs
...
const DAEMON_EXIT_MESSAGE =
  'daemon exited before reporting status (code=1, signal=none); see /Users/liudetao/Library/Application Support/Creator Studio Design/namespaces/release-stable/logs/daemon/latest.log for details';
const WEB_EXIT_MESSAGE =
  'daemon exited before reporting status (code=1, signal=none); see /Users/liudetao/Library/Application Support/Creator Studio Design/namespaces/release-stable/logs/web/latest.log for details';
...
describe('classifyStartupFailure', () => {
  ...
  it('classifies a Windows (backslash) web-start log path as web-start', () => {
    ...
    const winWebMessage =
      'daemon exited before reporting status (code=1, signal=none); see C:\\Users\\Alice\\AppData\\Roaming\\Creator Studio Design\\namespaces\\release-stable\\logs\\web\\latest.log for details';
...
describe('scrubUserPaths', () => {
  it('redacts the user home directory but keeps the rest of the path', () => {
    const scrubbed = scrubUserPaths(
      '/Users/liudetao/Library/Application Support/Creator Studio Design/namespaces/release-stable/logs/daemon/latest.log',
    );
...
describe('errno triplet privacy', () => {
  ...
  it('reduces a path-bearing Windows syscall to the operation token', () => {
    const err = Object.assign(new Error('spawn UNKNOWN'), {
      syscall: 'spawn C:\\Users\\Alice Smith\\AppData\\Creator Studio Design\\vela.exe',
    });
    expect(readErrnoFields(err).syscall).toBe('spawn');
  });
