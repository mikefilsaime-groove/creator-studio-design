...
async function createMacInstalledOuter(): Promise<{
  ...
  if (process.platform === "win32") {
    const launchPath = "/Applications/Creator Studio Design.app";
    const executablePath = `${launchPath}/Contents/MacOS/Creator Studio Design`;
    return {
      executablePath,
  ...
  roots.push(root);
  const launchPath = join(root, "Creator Studio Design.app");
  const executablePath = join(launchPath, "Contents", "MacOS", "Creator Studio Design");
...
async function createWindowsInstalledOuter(
  executableName = "Creator Studio Design.exe",
): Promise<{
  executablePath: string;
  inspectInstalledOuterPath?: InspectInstalledOuterPath;
  launchPath: string;
  ...
  if (process.platform !== "win32") {
    const executablePath = `C:\\Program Files\\Creator Studio Design\\${executableName}`;
...
describe("createObsoleteInstalledOuterRetirement", () => {
  it("stops only the exact installed outer root and its descendants", async () => {
    ...
    const snapshots = [
      ...
      snapshot(102, 101, `${launchPath}/Contents/Frameworks/Creator Studio Design Helper.app/Contents/MacOS/Creator Studio Design Helper`),
      snapshot(103, 102, "helper-child"),
      snapshot(104, 1, `${executablePath} Helper`),
      snapshot(105, 1, "/unrelated/Creator Studio Design"),
      snapshot(900, 1, "/payload/Creator Studio Design.app/Contents/MacOS/Creator Studio Design"),
    ];
    const stopProcesses = stopMock();
    const logger = { info: vi.fn(), warn: vi.fn() };
    const retire = createObsoleteInstalledOuterRetirement({
      currentExecutablePath: "/payload/Creator Studio Design.app/Contents/MacOS/Creator Studio Design",
      currentPid: 900,
      installedLaunchPath: launchPath,
      logger,
      payloadDesktopProcess: true,
      payloadExecutablePath: "/payload/Creator Studio Design.app/Contents/MacOS/Creator Studio Design",
      platform: "darwin",
    }, {
      inspectInstalledOuterPath,
  ...
  it("does nothing outside an exact payload desktop process", async () => {
    ...
    const retire = createObsoleteInstalledOuterRetirement({
      currentExecutablePath: "/installed/Creator Studio Design",
      currentPid: 900,
      installedLaunchPath: launchPath,
  ...
  it("rejects a symlinked install executable before enumerating processes", async () => {
    ...
    if (process.platform === "win32") {
      launchPath = "/Applications/Creator Studio Design.app";
      const executablePath = `${launchPath}/Contents/MacOS/Creator Studio Design`;
      ...
      roots.push(root);
      launchPath = join(root, "Creator Studio Design.app");
      ...
      await symlink(target, join(executableDirectory, "Creator Studio Design"));
    }
    const listProcessSnapshots = vi.fn(async () => []);
    const retire = createObsoleteInstalledOuterRetirement({
      currentExecutablePath: "/payload/Creator Studio Design",
      currentPid: 900,
      installedLaunchPath: launchPath,
      logger: { info: vi.fn(), warn: vi.fn() },
      payloadDesktopProcess: true,
      payloadExecutablePath: "/payload/Creator Studio Design",
      platform: "darwin",
    }, {
      inspectInstalledOuterPath,
  ...
  it("refuses to stop an outer tree that contains the current payload", async () => {
    ...
    const retire = createObsoleteInstalledOuterRetirement({
      ...
      listProcessSnapshots: async () => [
        ...
        snapshot(900, 800, "/payload/Creator Studio Design"),
...
describe("Windows obsolete installed outer retirement", () => {
  it("stops the revalidated exact installed outer root and its current descendants", async () => {
    ...
    const firstSnapshots = [
      ...
      snapshot(202, 201, "Creator Studio Design.exe --type=gpu-process"),
      snapshot(203, 202, "Creator Studio Design.exe --type=utility"),
      snapshot(204, 1, `"${executablePath}" od://project/123`),
      snapshot(205, 1, `${executablePath}.old`),
      snapshot(900, 1, "C:\\payload\\Creator Studio Design.exe"),
    ];
    const secondSnapshots = [
      snapshot(201, 1, `"${executablePath}"`),
      snapshot(202, 201, "Creator Studio Design.exe --type=gpu-process"),
      snapshot(206, 202, "Creator Studio Design.exe --type=renderer"),
      snapshot(204, 1, `"${executablePath}" od://project/123`),
      snapshot(205, 1, `${executablePath}.old`),
      snapshot(900, 1, "C:\\payload\\Creator Studio Design.exe"),
    ];
    ...
    const logger = { info: vi.fn(), warn: vi.fn() };
    const retire = createObsoleteInstalledOuterRetirement({
      currentExecutablePath: "C:\\PAYLOAD\\CREATOR STUDIO DESIGN.EXE",
      currentPid: 900,
      installedLaunchPath: launchPath,
      logger,
      payloadDesktopProcess: true,
      payloadExecutablePath: "c:\\payload\\Creator Studio Design.exe",
      platform: "win32",
    }, { inspectInstalledOuterPath, listProcessSnapshots, stopProcesses });
  ...
  it("rejects installed executables whose basename does not match the payload", async () => {
    const { inspectInstalledOuterPath, launchPath } = await createWindowsInstalledOuter("Not Creator Studio Design.exe");
    const listProcessSnapshots = vi.fn(async () => []);
    const stopProcesses = stopMock();
    const retire = createObsoleteInstalledOuterRetirement({
      currentExecutablePath: "C:\\payload\\Creator Studio Design.exe",
  ...
  it("refuses a revalidated outer tree that contains the current payload", async () => {
    ...
    const snapshots = [
      ...
      snapshot(900, 800, "C:\\payload\\Creator Studio Design.exe"),
    ];
  ...
  it("rejects a symlinked Windows executable before enumerating processes", async () => {
    const executablePath = "C:\\Program Files\\Creator Studio Design\\Creator Studio Design.exe";
    const listProcessSnapshots = vi.fn(async () => []);
    const stopProcesses = stopMock();
    const retire = createObsoleteInstalledOuterRetirement({
      currentExecutablePath: "C:\\payload\\Creator Studio Design.exe",
