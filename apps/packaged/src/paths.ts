  ...
  if (odDataDir) {
    ...
    if (!isAbs) {
      throw new PackagedPathAccessError(
        [
          "Creator Studio Design's packaged runtime requires OD_DATA_DIR to be an absolute path.",
          "",
    ...
    if (scopedNamespace) {
      if (scopedNamespace !== namespace) {
        throw new PackagedPathAccessError(
          [
            "Creator Studio Design's packaged runtime requires OD_DATA_DIR to target the active namespace.",
            "",
            `Configured value: ${odDataDir}`,
...
export function resolvePackagedNamespacePaths(
  ...
  // `~/Library/Application Support/Creator Studio Design Prerelease/` on mac. Custom
