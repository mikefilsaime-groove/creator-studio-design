...
export const PT_BR_SKILL_COPY: Record<string, { description?: string; examplePrompt?: string }> = {
  ...
  'agent-browser': {
    description:
      'CLI de automação de navegador para agentes de IA. Use quando o usuário precisar inspecionar,\ntestar ou automatizar o comportamento do navegador: navegar por páginas, preencher formulários,\nclicar em botões, capturar screenshots, extrair dados de páginas, ler o contexto selecionado\nda aba de navegador do Creator Studio Design, testar aplicativos web, fazer dogfooding de pré-visualizações\ndo Creator Studio Design, QA, caça a bugs ou revisão da qualidade do aplicativo. Prefira URLs locais de\npré-visualização do Creator Studio Design, a menos que o usuário peça explicitamente navegação externa.',
    examplePrompt:
  ...
  'ui-ux-pro-max': {
    description:
      'Entrada UI/UX Pro Max somente de catálogo. Os templates upstream completos, os dados e o fluxo de busca não estão incluídos no Creator Studio Design.',
    examplePrompt:
