...
// attribute the visit to Creator Studio Design (per-product revenue/traffic attribution).
//
...
export type RunFailureTitleKey =
  ...
  // S04 Creator Studio Design 智能体(主语固定,卡内一键授权)。
 ...
 * carries 〔switch to Creator Studio Design Cloud〕 in its primary slot (`cloudSwitchCta`),
...
/**
 * Rung 3 — "switch to the Creator Studio Design agent" — is not an answer for a run that
...
const AGENT_AGNOSTIC_DETAIL_FAILURE_UI: Record<string, RunFailureUi> = {
  ...
  // The fix is updating Creator Studio Design to a build that bundles a compatible
  ...
  //
  // No switch card: 推荐 Creator Studio Design 智能体 is reserved for「本地 agent 没登录」
...
function resolveRunFailureUiIgnoringSelfPromotion(
  ...
  if (agentId === CLOUD_NATIVE_AGENT_ID) {
    ...
    if (
      ...
      // 「Creator Studio Design 尚未登录」—— 主语固定,和 S02 那句**不是**同一句话:
