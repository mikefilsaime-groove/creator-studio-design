...
function truncateForTranscript(content: string): string {
  ...
  return `${content.slice(0, MAX_TRANSCRIPT_MESSAGE_CHARS)}\n\n[Creator Studio Design truncated ${omitted} chars from this prior message before sending it to the agent. Full content remains in persisted history.]`;
}
...
function buildPriorRunContextWarning(history: ChatMessage[]): string | null {
  ...
  return [
    '## context warning',
    `Creator Studio Design detected ${notes.join(', ')}.`,
 ...
 * answered and Creator Studio Design could not record the answer". S21, the nearest,
...
export const STRATEGY_TASK_BLOCKED_MESSAGE =
  "The agent's reply did not carry the machine-readable state Creator Studio Design needs "
...
const AMR_OPENCODE_INCOMPLETE_MESSAGE =
  'Creator Studio Design started, but the run did not complete. Please retry or check the run details for the session stream error.';
...
function daemonCreateRunError(response: Response, responseText: string): Error {
  ...
  const error = new Error(apiError.message || `Creator Studio Design service returned ${response.status}`) as Error & {
    code?: string;
    requestId?: string;
    retryable?: boolean;
    status?: number;
...
function formatOpenCodeSessionError(value: unknown): string | null {
  ...
  if (statusCode === 404) {
    return 'The model service returned 404 Not Found for the configured runtime endpoint. Check the Creator Studio Design link URL or model route.';
  }
  if (statusCode === 401 || statusCode === 403) {
    return 'Creator Studio Design authentication failed. Please sign in again or refresh the runtime key.';
  }
  if (statusCode === 429) {
