...
function buildContributePrompt(folderPath: string): string {
  return [
    ...
    `Manifest: \`${folderPath}/open-design.json\``,
    '',
    'Run this deterministic Creator Studio Design CLI workflow from the current project workspace:',
    '',
...
// Creator Studio Design issue URL and belongs to the "Creator Studio Design PR" button. Before
...
function buildPublishPrompt(folderPath: string): string {
  return [
    ...
    `Manifest: \`${folderPath}/open-design.json\``,
    '',
    'This is the **repository publish** action, NOT the registry-submission action — do NOT route through `od plugin publish --to open-design`. That command emits an Creator Studio Design issue URL and belongs to the "Creator Studio Design PR" button.',
    '',
    'Run this deterministic Creator Studio Design CLI workflow from the current project workspace:',
    '',
