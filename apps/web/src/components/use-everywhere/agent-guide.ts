...
// everything it needs to install Creator Studio Design, expose it as MCP, and
...
export interface AgentGuideOptions {
   ...
   */
  cliHint?: string;
  /** Optional Creator Studio Design version/channel; surfaced in the header for support tickets. */
  versionHint?: string;
}
...
export function buildAgentGuideMarkdown(options: AgentGuideOptions = {}): string {
  ...
  lines.push('# Creator Studio Design — agent setup guide');
  lines.push('');
  lines.push(
    'You are a coding agent (Claude Code, Codex, Cursor, openclaw, hermes, or similar). ' +
      'The user wants Creator Studio Design wired into your workflow. Creator Studio Design is a local ' +
  ...
  if (options.versionHint) {
    lines.push('');
    lines.push(`> Reported Creator Studio Design version: \`${options.versionHint}\``);
  }
  lines.push('');
  ...
  lines.push('   ```');
  lines.push('');
  lines.push('   If it 404s or times out, ask the user to run `pnpm tools-dev` (dev) or open the Creator Studio Design app (packaged).');
  lines.push('');
  if (installInfo) {
