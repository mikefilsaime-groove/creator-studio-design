// Content fixtures for the "Use Creator Studio Design everywhere" guide modal.
//
...
export const GUIDE_SECTIONS: GuideSection[] = [
  {
    id: 'overview',
    tabLabel: 'Overview',
    heading: 'Creator Studio Design works wherever your agent works',
    intro:
      'Creator Studio Design is more than a window — it is a local privileged daemon ' +
    ...
    bullets: [
      ...
      'MCP server — wires Creator Studio Design as a Model Context Protocol server so any MCP-capable agent can list skills, run scenarios, and read artifacts.',
      'HTTP API — `http://127.0.0.1:7456/api/*` REST + SSE endpoints; the same surface the web UI uses.',
      'Skills — drop-in `SKILL.md` packs (Claude-compatible) that any agent already on your PATH can invoke without Creator Studio Design at all.',
    ...
    snippets: [
      ...
      },
      {
        ...
        body:
          ...
          '# Then start Creator Studio Design in the shell you normally use for dev:\n' +
          'pnpm tools-dev',
      },
    ],
    footer:
  ...
  },
  {
    id: 'cli',
    tabLabel: 'CLI · od',
    heading: 'Drive Creator Studio Design from any shell',
    intro:
  ...
  },
  {
    id: 'mcp',
    tabLabel: 'MCP server',
    heading: 'Expose Creator Studio Design as an MCP server to any coding agent',
    intro:
      'Creator Studio Design ships with a Model Context Protocol server (`od mcp`) ' +
      'that lets any MCP-capable client — Cursor, Claude Code, Antigravity, ' +
      'VS Code Copilot Chat, openclaw, hermes — discover Creator Studio Design tools ' +
    ...
    footer:
      'In the Creator Studio Design app, open Settings → Integrations to copy a ' +
  ...
  },
  {
    ...
    heading: 'Drop-in Skills for any agent — even without Creator Studio Design running',
    intro:
      'A Skill is a directory with a Claude-compatible `SKILL.md` ' +
      '(YAML front-matter + body). Creator Studio Design extends the format with the ' +
      '`od:` namespace (`mode`, `preview`, `design_system`, `inputs`, …) so ' +
      'the same artifact can be used both inside Creator Studio Design and by a vanilla ' +
    ...
    snippets: [
      {
        label: 'Minimal SKILL.md (Claude-compatible front matter + Creator Studio Design extras)',
        language: 'yaml',
        body:
          '---\n' +
      ...
      },
      {
        ...
        body:
          ...
          '# Shell 1: start Creator Studio Design after ingesting.\n' +
          'pnpm tools-dev\n' +
