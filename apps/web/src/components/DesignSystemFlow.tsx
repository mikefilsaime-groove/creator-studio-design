...
function buildFallbackDesignMdFromState(state: SetupState): string {
  ...
  const overview =
    ...
    || state.notes.trim()
    || sourceNotes
    || 'Design system generated from source material supplied in Creator Studio Design.';
  return [
    '---',
...
function buildCreationAgentPrompt(
  ...
  return [
    ...
    state.codeFolders.length
      ? `Read the linked local code folders that Creator Studio Design attached to this project: ${state.codeFolders.join(', ')}. Treat them as source context only unless the user asks you to edit them.\n\n${localFolderRunbook}`
      : '',
...
function buildSourceContextManifest(
  ...
  sections.push(
    ...
    '- README.md and SKILL.md should make the extracted system reusable as a real Creator Studio Design design-system package.',
