...
function SkillPluginCandidateCard({
  ...
  async function share(action: "contribute-open-design") {
    ...
    try {
      ...
      setNotice({
        message: `Creator Studio Design contribution task started for ${data?.path ?? "the draft"}.`,
      });
    } catch (err) {
      setNotice({ message: err instanceof Error ? err.message : String(err) });
    } finally {
...
interface Props {
  ...
  // Click handler for the post-completion "Share to Creator Studio Design" submission
...
function hasPluginFinalActionHint(content: string): boolean {
  return /\b(Add to My plugins|Creator Studio Design PR|Publish repo|plugin publish|ready to publish|ready to add)\b/i.test(
    content,
  );
}
