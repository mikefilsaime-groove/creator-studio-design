...
describe('WhatsNewPopup version resolution', () => {
  ...
  it('never paints the placeholder while /api/version is still in flight', async () => {
    ...
    expect(screen.getByText(`Creator Studio Design ${SHOW_PAYLOAD.version} is here`)).toBeTruthy();
  });
  ...
  it('switches to the running version once /api/version resolves', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText(`Creator Studio Design ${RUNNING_APP_VERSION} is here`)).toBeTruthy();
    });
    expect(screen.queryByText(`Creator Studio Design ${SHOW_PAYLOAD.version} is here`)).toBeNull();
  });
