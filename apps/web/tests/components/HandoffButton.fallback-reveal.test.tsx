...
describe('HandoffButton zero-editors fallback', () => {
  ...
  it('copies a framework-specific CLI handoff prompt with the local project path', async () => {
    ...
    // The "Creator Studio Design Cloud website" link was removed from the CLI tab
    // (acceptance #101); the CLI agent cards remain the surface here.
    expect(screen.queryByRole('link', { name: /打开 Creator Studio Design Cloud 官网/ })).toBeNull();
    expect(screen.getByTestId('handoff-cli-item-amr').textContent).toContain('Creator Studio Design');
