...
describe('AmrAccountControl', () => {
  it('renders the compact signed-out status and sign-in action', () => {
    ...
    expect(
      screen.getByRole('group', { name: 'Creator Studio Design Cloud account status' }),
    ).toBeTruthy();
...
describe('AmrLoginPill', () => {
  ...
  it('passes the Creator Studio Design device id in login attribution when metrics consent is enabled', async () => {
    const fetchMock = vi.fn(async (input, init) => {
      const url = typeof input === 'string' ? input : (input as URL).toString();
      if (url.endsWith('/api/integrations/vela/status')) {
        return jsonResponse({
  ...
  // Creator Studio Design agent card's "Authorize" action both render (SettingsDialog
