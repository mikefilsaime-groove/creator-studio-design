...
/**
 * The signed-out rail's bottom "Creator Studio Design Cloud" callout is the ONLY entry
...
describe('RailAccountSyncTip', () => {
  ...
  it('renders an inert status readout instead of leaving the slot blank', async () => {
    ...
    expect(status.textContent).toContain('Creator Studio Design Cloud');
  ...
  it('renders an avatar + name skeleton shaped like the real account row, not a spinner card', async () => {
    ...
    // The old card rendered a visible "Creator Studio Design Cloud" / "Loading…" pair
