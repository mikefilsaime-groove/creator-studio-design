...
const defaultMessages: MessageCenterMessage[] = [
  { id: 'release', audienceType: 'global', typeName: 'Product update', title: 'Creator Studio Design 0.14 is available', body: 'The new release is ready.', ctaLabel: 'View update', ctaUrl: 'https://open-design.ai/update', publishedAt: '2026-07-16T12:00:00.000Z', readAt: null },
...
describe('MessageCenter', () => {
  ...
  it('shows read and unread messages in one flat list without filter or bulk actions', async () => {
    ...
    expect(within(dialog).queryByText('Creator Studio Design updates, platform announcements, and account notices.')).toBeNull();
  ...
  it('reveals the media on expand with type and date below it', async () => {
    ...
    renderMessageCenter();
    const dialog = await openCenter();
    const row = within(dialog).getByRole('button', { name: /Creator Studio Design 0\.14 is available/ });
    const title = within(row).getByText('Creator Studio Design 0.14 is available');
  ...
  it('keeps visible account messages when locale changes and the follow-up sync fails', async () => {
    ...
    await openCenter();
    await waitFor(() => expect(screen.getByText('Creator Studio Design 0.14 is available')).toBeTruthy());
