...
describe('DesignSystemCreationFlow', () => {
  ...
  it.skip('opens the project as soon as the workspace exists and prepares the first chat task afterward', async () => {
    ...
    await waitFor(() => expect(mocks.patchProject).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        pendingPrompt: expect.stringContaining('Create this project as a complete Creator Studio Design design system workspace.'),
      }),
    ));
  ...
  it.skip('adds website source links with Enter and keeps them out of GitHub intake', async () => {
    const system: DesignSystemDetail = {
      ...
      title: 'Creator Studio Design Website Design System',
      category: 'Custom',
      summary: 'Creator Studio Design website source.',
      swatches: [],
      surface: 'web',
      body: '# Creator Studio Design Website Design System\n',
      source: 'user',
      status: 'draft',
      isEditable: true,
    ...
    const project: Project = {
      ...
      name: 'Creator Studio Design Website Design System',
      skillId: null,
      designSystemId: system.id,
      createdAt: 1,
      updatedAt: 1,
    ...
    fireEvent.change(screen.getByPlaceholderText(/Mission Impastabowl/i), {
      target: { value: 'Creator Studio Design website source' },
    });
    continueToGeneration();
    continueToGeneration();
  ...
  it.skip('allows GitHub repo links without Composio by using local GitHub intake', () => {
    ...
    expect(screen.getByText('Creator Studio Design account')).toBeTruthy();
  ...
  it('keeps a manual GitHub authorization action when the automatic popup is blocked', async () => {
    ...
    mocks.connectConnector.mockResolvedValue({
      ...
      error: 'Popup blocked. Allow popups for Creator Studio Design and try again.',
    });
    ...
    try {
      ...
      expect(screen.getByText('Popup blocked. Allow popups for Creator Studio Design and try again.')).toBeTruthy();
