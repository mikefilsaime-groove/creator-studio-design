...
describe('Creator Studio Design settings restrictions', () => {
  ...
  it('ships Local CLI only and removes BYOK, media-provider, and pet settings', () => {
    ...
    expect(screen.queryByText(en['settings.cloudCalloutTitle'])).toBeNull();
    expect(screen.queryByText(en['settings.cloudCalloutBody'])).toBeNull();
    expect(screen.queryByRole('button', { name: en['settings.cloudCalloutButton'] })).toBeNull();
  });
});
