...
describe('AssistantMessage — chat file-link routing (#1239)', () => {
  ...
  it('navigates in-place to app file URLs for a different project instead of opening a new window', () => {
    ...
    // Not the current workspace's file — but it IS an Creator Studio Design file, so
  ...
  it('routes angle-wrapped local paths containing spaces through onRequestOpenFile', () => {
    const onRequestOpenFile = vi.fn();
    const projectDir = '/Users/me/Library/Application Support/Creator Studio Design Beta/projects/project-1';
    const { container } = render(
      <AssistantMessage
        message={messageWithText(
