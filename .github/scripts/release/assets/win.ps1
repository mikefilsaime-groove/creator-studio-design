...
$sourceInstaller = Join-Path $builderDir "Creator Studio Design-${env:TOOLS_PACK_NAMESPACE}-setup.exe"
$sourceZip = Join-Path $builderDir "Creator Studio Design-${env:TOOLS_PACK_NAMESPACE}-portable.zip"
...
$versionedInstaller = "creator-studio-design-${env:RELEASE_VERSION}$assetSuffix-win-x64-setup.exe"
$versionedZip = "creator-studio-design-${env:RELEASE_VERSION}$assetSuffix-win-x64-portable.zip"
...
$releaseNotes = if ([string]::IsNullOrWhiteSpace($env:RELEASE_NOTES)) {
  "Creator Studio Design ${env:RELEASE_VERSION}$assetSuffix"
} else {
  $env:RELEASE_NOTES
}
