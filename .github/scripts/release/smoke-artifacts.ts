...
function artifactDestination(target: ReleaseTarget, toolsPackDir: string, namespace: string): string {
  ...
  if (target === "win_x64") {
    return join(toolsPackDir, "out", "win", "namespaces", namespace, "builder", `Creator Studio Design-${token}-setup.exe`);
  }
  if (target === "linux_x64") {
    return join(toolsPackDir, "out", "linux", "namespaces", namespace, "builder", `Creator Studio Design-${token}.AppImage`);
  }
  return join(toolsPackDir, "out", "mac", "namespaces", namespace, "dmg", `Creator Studio Design-${token}.dmg`);
}
