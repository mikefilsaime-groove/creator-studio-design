...
Local packaging control plane for Creator Studio Design.
...
Public release bundles keep channel-distinct identities: `Creator Studio Design.app`, `Creator Studio Design Beta.app`,
`Creator Studio Design Prerelease.app`, or `Creator Studio Design Preview.app`. Local `tools-pack install` adds the developer
...
- Finder/manual launches on macOS may not inherit your shell-managed `PATH`. If packaged Creator Studio Design cannot detect agent CLIs that work in Terminal, expose those binaries to the GUI login environment or launch the packaged app from a shell session that already sees them.
