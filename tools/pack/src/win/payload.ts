...
export async function buildWinLauncherPayloadArchive(
  ...
  const writeOverlay = async (input: { includeExecutable: boolean }): Promise<void> => {
    ...
    if (input.includeExecutable) {
      await cp(join(builtApp.unpackedRoot, "Creator Studio Design.exe"), join(overlayRoot, "payload", "Creator Studio Design.exe"));
    }
    await writeFile(
...
export async function validateWinLauncherPayloadArchive(input: {
  ...
  try {
    ...
    requirePayloadManifestValue(manifest.entry?.executable, "entry.executable", "payload/Creator Studio Design.exe");

    await stat(join(extractRoot, archiveRelativePath("payload/Creator Studio Design.exe")));
