...
export function renderPackagedMainEntry(usePrebundle: boolean): string {
  ...
  const message = (missingModule
    ? "Creator Studio Design could not load a required application file. "
    : "Creator Studio Design could not start. ") +
    "Download and reinstall the latest Creator Studio Design installer for the same release channel. Keep your application data. If the problem persists, contact support with this error.";
  ...
  try {
    ...
    if (process.versions.electron && process.env.ELECTRON_RUN_AS_NODE !== "1" && !process.argv.includes("--headless")) {
      require("electron").dialog.showErrorBox("Creator Studio Design could not start", message);
    }
  } catch (reportError) {
