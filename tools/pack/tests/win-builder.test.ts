...
function createPaths(root: string): WinPaths {
  ...
  return {
    ...
    blockmapPath: join(namespaceRoot, "builder", "Creator Studio Design-second-setup.exe.blockmap"),
    ...
    exePath: join(namespaceRoot, "builder", "Creator Studio Design-second.exe"),
    installDir: join(namespaceRoot, "runtime", "install", "Creator Studio Design"),
    installedExePath: join(namespaceRoot, "runtime", "install", "Creator Studio Design", "Creator Studio Design.exe"),
    ...
    launcherPayloadPath: join(namespaceRoot, "payload", "Creator Studio Design-second-payload.7z"),
    ...
    setupPath: join(namespaceRoot, "builder", "Creator Studio Design-second-setup.exe"),
    setupZipPath: join(namespaceRoot, "builder", "Creator Studio Design-second-portable.zip"),
    ...
    uninstallerPath: join(namespaceRoot, "runtime", "install", "Creator Studio Design", "Uninstall.exe"),
...
describe("materializeCachedUnpackedForInstaller", () => {
  it("overwrites cached packaged config and app package version", async () => {
    ...
    try {
      ...
      await writeFile(join(cachedUnpackedRoot, "Creator Studio Design.exe"), await createVersionedExecutable("0.5.0-beta.1"));
      await writeFile(
      ...
      await expect(readWinExecutableVersionSnapshot(join(paths.unpackedRoot, "Creator Studio Design.exe"))).resolves.toMatchObject({
...
async function createVersionedExecutable(packagedVersion: string): Promise<Buffer> {
  ...
  version.setStringValues(
    ...
    {
      FileDescription: "Creator Studio Design",
