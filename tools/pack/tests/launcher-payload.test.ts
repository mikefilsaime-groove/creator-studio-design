...
function createWinPaths(root: string, namespace: string): WinPaths {
  ...
  return {
    ...
    blockmapPath: join(namespaceRoot, "builder", "Creator Studio Design-release-beta-win-setup.exe.blockmap"),
    ...
    exePath: join(namespaceRoot, "builder", "Creator Studio Design-release-beta-win.exe"),
    installDir: join(namespaceRoot, "runtime", "install", "Creator Studio Design Beta"),
    installedExePath: join(namespaceRoot, "runtime", "install", "Creator Studio Design Beta", "Creator Studio Design.exe"),
    ...
    launcherPayloadPath: join(namespaceRoot, "payload", "Creator Studio Design-release-beta-win-payload.7z"),
    ...
    setupPath: join(namespaceRoot, "builder", "Creator Studio Design-release-beta-win-setup.exe"),
    setupZipPath: join(namespaceRoot, "builder", "Creator Studio Design-release-beta-win-portable.zip"),
    ...
    uninstallerPath: join(namespaceRoot, "runtime", "install", "Creator Studio Design Beta", "Uninstall.exe"),
    ...
    unpackedExePath: join(namespaceRoot, "builder", "win-unpacked", "Creator Studio Design.exe"),
...
async function writeFakeWinUnpackedApp(root: string, namespace: string, version: string): Promise<{
  ...
  await writeFile(join(paths.unpackedRoot, "Creator Studio Design.exe"), "fake executable\n", "utf8");
  await writeFile(
  ...
  return {
    builtApp: {
      ...
      executablePath: join(paths.unpackedRoot, "Creator Studio Design.exe"),
      source: "namespace",
...
describe("tools-pack launcher payload archives", () => {
  it("builds channel and namespace scoped payload manifests for both desktop platforms", () => {
    ...
    expect(buildMacLauncherPayloadManifest({
      ...
      appBundleName: "Creator Studio Design Beta.app",
      channel: "beta",
      entry: {
  ...
  it.skipIf(process.platform !== "darwin")("creates a mac payload zip with bootstrap-readable contents", async () => {
    ...
    try {
      ...
      expect(manifest.appBundleName).toBe("Creator Studio Design Beta.app");
      expect(manifest.entry.executable).toBe("payload/Creator Studio Design Beta.app/Contents/MacOS/Creator Studio Design Beta");
  ...
  it.skipIf(process.platform !== "win32")("creates a Windows payload 7z with bootstrap-readable contents", async () => {
    ...
    try {
      ...
      expect(manifest.entry.executable).toBe("payload/Creator Studio Design.exe");
      expect(manifest.version).toBe(version);
      await expectPathExists(join(extractRoot, "payload", "Creator Studio Design.exe"));
