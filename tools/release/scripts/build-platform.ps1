...
function Validate-WinLauncherPayloadArchive([string]$PayloadPath, [string]$ExpectedVersion, [string]$Label) {
  ...
  try {
    ...
    Test-JsonString $manifest.entry.executable "entry.executable" "payload/Creator Studio Design.exe"

    $entryPath = Join-Path $extractRoot (Convert-ArchiveRelativePath "payload/Creator Studio Design.exe")
