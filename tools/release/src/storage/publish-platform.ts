...
function targetConfig(): TargetConfig {
  if (target === "mac_arm64" || target === "mac_x64") {
    ...
    const dmg = `creator-studio-design-${releaseVersion}${assetSuffix}-mac-${arch}.dmg`;
    const zip = `creator-studio-design-${releaseVersion}${assetSuffix}-mac-${arch}.zip`;
    ...
    if (artifactMode === "dmg-and-payload" || artifactMode === "all") {
      const payload = `creator-studio-design-${releaseVersion}${assetSuffix}-mac-${arch}-payload.zip`;
  ...
  if (target === "win_x64") {
    const installer = `creator-studio-design-${releaseVersion}${assetSuffix}-win-x64-setup.exe`;
    const payload = `creator-studio-design-${releaseVersion}${assetSuffix}-win-x64-payload.7z`;
    const portableZip = `creator-studio-design-${releaseVersion}${assetSuffix}-win-x64-portable.zip`;
  ...
  const appImage = `creator-studio-design-${releaseVersion}${assetSuffix}-linux-x64.AppImage`;
  return {
    arch: "x64",
